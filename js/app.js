...
function addToDo(toDo, id, done, trash) {
  ...
  const text = `<li class="item">
        ...
        <input class="text ${LINE}" value="${toDo}" job="modif"> </input>
...
function replaceTodo(element) {
  element.addEventListener("keyup", function (event) {
    if (event.keyCode == 13) {
      // recupere l'input
      LIST[element.nextElementSibling.id].name =
...
// cible les elements que contient la list (parent element) dynamiquement
